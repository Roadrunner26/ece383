...
In order to showcase functionality, I would run the file on my monitor and change the trigger voltage up and down to see if the channels would successfully rise on the trigger. For gatecheck 3, I would try to see if the data would scroll when I would switch on and off simlive. I knew that this lab was working when I could change the input frequency from the lab 2 page and see if it would still trigger and appear stationary on the screen. At this time, triggerTime has no function.
